...
- **Seats reach each other's work**: verified replies are auto-committed on the seat's branch and the event names the sha; every claude seat can read sibling worktrees (`runs/<run>/workspaces/<seat>`) and `git show md-agent/<run>/<seat>:<path>`. When a seat must consume another's output, the dispatch should name the branch or path — and go out only after the producer's reply lands.
- **Readiness**: a seat ends a reply that is ready for the verify command with the line `VERIFY-READY`; interim slice reports are attached, not bounced. A turn stopped by `turnBudgetUsd`/`turnTimeoutSec` comes back as `[TURN CAPPED]` with its spend booked and its partial work noted — re-dispatch smaller.
- **Script mode**: `"script": "./drive.mjs"` replaces the model orchestrator with a JS driver — `h.dispatch(seat, ask)` resolves with the reply, `Promise.all` batches asks into one turn, `h.read(seat, path)` reads a sibling's file, the returned string is the completion reason. Use it whenever the phases are known up front; it removes sequencing mistakes and costs no orchestrator turns. `"setup": "npm ci"` runs once per fresh worktree (and in the merged tree) for checks that need installed dependencies.
- **Read discipline**: every seat is told never to print whole files, to read with offsets/line budgets, and to batch many checks into one grep or script (tool output was 76% of seat context in the field runs). Give judging seats `readOnly: true` (Read/Grep/Glob only, pair with `verify: false`). Claude turns carry a per-tier USD cap by default (opus 5 / sonnet 2.5 / haiku 1; `turnBudgetUsd` overrides, 0 = none); recycle-by-size fires at 120k resident tokens.
- **Prefix**: seats load no skills by default (`skills: true` restores); `projectInstructions: false` drops the project's CLAUDE.md from a read-only seat's every call (~20k tokens on a large one). The orchestrator never loads it.
...
- **Alternatives:** the other provider · tier pairs with what each is good for and what it costs:
  - `claude · opus` — every seat that JUDGES: review, challenge, audit, drift/gap analysis, anything whose output is a conclusion. Measured 2026-09-05 on one review: opus found 20 verified findings; sonnet found 3–5 of them and misrated those. Split judging across parallel opus seats for wall-clock, never down-tier it for cost. $5/$25 per M tokens.
  - `claude · sonnet` — verification-shaped and engineering work a command checks: applying a known change, extraction to a schema, citation checking. NOT a reviewer over conclusions, and as a challenger it verifies citations, not conclusions (it passed false positives opus had struck). $2/$10.
  - `claude · haiku` — mechanical, narrow, high-volume work behind a verify: enumeration, tables, counts. $1/$5.
...
```json
{
  ...
  "roles": [
    ...
    { "name": "reviewer", "description": "Read the worker's diff for behavior drift; say no when it should.",     "provider": "claude", "model": "opus",   "readOnly": true, "verify": false }
  ],
